...
话题规则：
1. candidateTopics 只包含本次尚未汇总的新消息候选；不要要求更多上下文，不要把历史或旧候选重新计数。
2. 如果 candidateTopics 是全新话题，请生成新的 id、title、summary、sourceMessageIds 和 sourceChats。
3. 话题表示多人或多轮围绕同一主题的讨论，不要按群名/联系人名简单排行。
...
3. 返回更新后的完整 topics：包括未变化的 existingTopics，以及合并/新增后的话题；最多返回 12 个。
4. 合并候选时不能只因为共享“买”“采购”“东西”“确认”“处理”“安排”等泛动作词就合并。
5. 如果不同 sourceChats 的 snippets 只表现出相似动作、但人物关系、业务场景或办理对象不同，必须拆成不同话题。例如“女朋友让我买东西”和“公司群讨论采购是否已买”不能合并。
...
11. scoreMultiplier 范围 0~1.5；confidence 范围 0~1。没有关键词识别消息列表时，keywords 返回空数组或省略。

旧关键词处理规则：
1. 不要参考本地识别词，不要合并旧关键词；只按 keywordRefine.messages 重新判断本次关键词。
2. AI 返回的 keywords 会全量替换本地关键词结果：应该输出本次判断后的完整有效关键词集合；不确定或不合格的词不要为了保留旧结果而返回。
...
function isKnownDefaultAnalysisPrompt(prompt: string): boolean {
  ...
  const missingWechatSystemAccountFilter = trimmed.includes("管理介入/情绪风险规则")
    && trimmed.includes("批内去重与历史参考规则")
    && (!trimmed.includes("filehelper") || !trimmed.includes("notification_messages"));
  const missingSingleChatFileReplyRule = trimmed.includes("待我回复")
    && trimmed.includes("批内去重与历史参考规则")
    && !trimmed.includes("单聊里对方发送文件");
  ...
  return trimmed === defaultAnalysisPrompt.trim()
    ...
    && (
      ...
      || missingWechatSystemAccountFilter
      || missingSingleChatFileReplyRule
    );
}

function isKnownDefaultSummaryPrompt(prompt: string): boolean {
  const trimmed = prompt.trim();
  const missingWechatSystemAccountFilter = trimmed.includes("旧话题去重合并规则")
    && trimmed.includes("关键词词云识别规则")
    && (!trimmed.includes("filehelper") || !trimmed.includes("notification_messages"));
  ...
  return trimmed === defaultSummaryPrompt.trim()
    ...
    && (
      ...
      || trimmed.includes("不要参考本地识别词，不要合并旧关键词")
      || !trimmed.includes("旧话题去重合并规则")
      || missingWechatSystemAccountFilter
    );
}
