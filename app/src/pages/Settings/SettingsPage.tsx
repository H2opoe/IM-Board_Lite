...
export function SettingsPage() {
  ...
  const isAnalysisPromptDefault = config.analysisPrompt === defaultConfig.analysisPrompt;
  const isSummaryPromptDefault = config.summaryPrompt === defaultConfig.summaryPrompt;
            ...
            <div className="field-row local-model-row">
              ...
              <div className={localDeepseek?.installed ? "local-model-box installed" : "local-model-box"}>
                <div className="local-model-main">
                  ...
                  {systemCapabilities && (
                    <small className={systemCapabilities.localModelSupported ? "hardware-status" : "hardware-status warning"}>
                      {systemCapabilities.architecture} · {systemCapabilities.logicalCpuCores}核 · {formatBytes(systemCapabilities.totalMemoryBytes)}内存 · {formatBytes(systemCapabilities.availableDiskBytes)}可用
                      {systemCapabilities.warnings[0] ? ` · ${systemCapabilities.warnings[0]}` : ""}
                    </small>
                  )}
              ...
              {!isLocalProvider && (
                <label className="field-row">
                  ...
                  <input
                    type="password"
                    value={config.apiKey}
                    onChange={(event) => updateConfigDraft({ ...config, apiKey: event.target.value, clearApiKey: false })}
                    placeholder={config.apiKeyConfigured ? "已安全保存，留空则保持不变" : "sk-..."}
                  />
                  {config.apiKeyConfigured && (
                    <button
                      type="button"
                      className="secondary-button slim-button"
                      onClick={() => updateConfigDraft({ ...config, apiKey: "", apiKeyConfigured: false, clearApiKey: true })}
                    >
                      清除已保存密钥
                    </button>
                  )}
            ...
            <div className="prompt-panel-body">
              <div className="prompt-editor">
                <div className="prompt-title">
                  ...
                  <button
                    ...
                    onClick={() => updateConfigDraft({ ...config, analysisPrompt: defaultConfig.analysisPrompt, analysisPromptCustom: false })}
                ...
                <label className="field-row prompt-field">
                  ...
                  <textarea
                    ...
                    placeholder={defaultConfig.analysisPrompt}
                  />
                </label>
              </div>
              ...
              <div className="prompt-editor">
                <div className="prompt-title">
                  ...
                  <button
                    className="secondary-button"
                    onClick={() => updateConfigDraft({ ...config, summaryPrompt: defaultConfig.summaryPrompt, summaryPromptCustom: false })}
                ...
                <label className="field-row prompt-field">
                  ...
                  <textarea
                    ...
                    placeholder={defaultConfig.summaryPrompt}
                  />
                </label>
              </div>
            </div>
