...
export async function initializeAutoSyncFrequencyMinutes(legacyMinutes: number): Promise<number> {
  if (isDemoMode()) return legacyMinutes;
  if (isTauri) return invoke("initialize_auto_sync_frequency_minutes", { legacyMinutes });
  return requireTauri("读取后台同步频率");
}
