...
async fn await_ai_call_with_runtime_recovery<T, F>(
    app: &tauri::AppHandle,
    state: &State<'_, AppState>,
...
where
    F: for<'a> FnMut(
        &'a crate::storage::models::AiConfig,
    ) -> futures::future::BoxFuture<'a, anyhow::Result<T>>,
{
    let runtime_config = ai_commands::runtime_config_for_analysis(app, state, config)
        .await
    ...
    let first_result = await_ai_call_or_cancel(state, build_call(&runtime_config)).await;
    ...
    let recovered_config = ai_commands::recover_runtime_config_for_analysis(app, state, config)
        .await
    ...
    await_ai_call_or_cancel(state, build_call(&recovered_config)).await
}
