...
pub(crate) fn classify_message_noise(
    ...
    if contains_interaction_system_notice(trimmed) {
        return Some(MessageNoiseKind::InteractionSystem);
    }
...
pub(crate) fn contains_client_compatibility_notice(content: &str) -> bool {
    ...
    ((compact.contains("微信版本不支持") || compact.contains("当前版本不支持"))
        && (compact.contains("展示内容")
            ...
            || compact.contains("查看")
            || compact.contains("升级")))
        || (compact.contains("请升级微信") && compact.contains("查看"))
}
...
fn contains_interaction_system_notice(content: &str) -> bool {
    let compact = compact_text(content);
...
mod tests {
    ...
    fn classifies_representative_group_noise() {
        ...
        assert_eq!(
            classify_message_noise(Some("text"), "请升级微信查看", true),
            Some(MessageNoiseKind::ClientCompatibility)
        );
        assert_eq!(
            classify_message_noise(Some("text"), "张三拍了拍李四", true),
            Some(MessageNoiseKind::InteractionSystem)
        );
        assert_eq!(
