...
pub fn get_config(conn: &rusqlite::Connection) -> anyhow::Result<AiConfig> {
    ...
    let mut config = row.unwrap_or_else(default_config);
    config.api_key = crate::security::credentials::resolve_ai_api_key(config.api_key)?;
    Ok(config)
}
...
pub fn save_config(conn: &rusqlite::Connection, config: AiConfig) -> anyhow::Result<AiConfig> {
    ...
    if normalized.api_key.trim().is_empty() {
        crate::security::credentials::delete_ai_api_key()?;
    } else {
        crate::security::credentials::store_ai_api_key(&normalized.api_key)?;
    }
    conn.execute(
...
pub(crate) fn normalize_config(config: AiConfig) -> AiConfig {
    // 后端是内置提示词的唯一来源；只有明确标记为自定义的内容才不跟随内置版本升级。
...
pub(crate) fn apply_hardware_batch_limit(config: &mut AiConfig, recommended_limit: i64) {
    if is_managed_local_deepseek(config) {
        config.analysis_batch_size = config
            .analysis_batch_size
            .min(recommended_limit.max(MIN_ANALYSIS_BATCH_MESSAGES));
    }
}
