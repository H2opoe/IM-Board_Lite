...
pub(crate) fn normalize_message(
    ...
    let content =
        first_string(value, &["content", "text", "message", "msg", "summary"]).unwrap_or_default();
    if content.trim().is_empty() {
        return None;
    }

    let chat_id = first_string(value, &["chatId", "chat_id", "roomId", "room_id"])
    ...
    let local_id = first_string(
        value,
        &["localId", "local_id", "msgId", "msg_id", "serverId", "id"],
    );
...
pub(crate) fn dedupe_sessions(sessions: Vec<serde_json::Value>) -> Vec<serde_json::Value> {
    ...
    for session in sessions {
        let Some(chat_id) = first_string(
            &session,
            &["chatId", "chat_id", "username", "userName", "id"],
        ) else {
            continue;
        };
...
mod tests {
    ...
    #[test]
    fn normalize_text_message_uses_fallback_chat_metadata() {
        ...
        assert_eq!(message.chat_id, "chat-customer");
        assert_eq!(message.chat_name, "客户群");
        assert_eq!(message.sender_name, "客户");
    }
}
