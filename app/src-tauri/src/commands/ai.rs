...
use crate::storage::models::{
    AiConfig, AiConfigInput, AiConfigView, LocalModelDownloadProgress, LocalModelStatus,
};
...
const LOCAL_DEEPSEEK_EXPECTED_SHA256: &str =
...
#[tauri::command]
pub fn get_ai_config(state: State<'_, AppState>) -> Result<AiConfigView, String> {
    ...
    apply_managed_runtime_endpoint(&state, &mut config);
    Ok(config.into())
}

#[tauri::command]
pub fn get_default_ai_config(state: State<'_, AppState>) -> AiConfigView {
    let mut config = ai::default_config();
    config.analysis_batch_size = state.system_capabilities.recommended_analysis_batch_size;
    apply_managed_runtime_endpoint(&state, &mut config);
    config.into()
}
...
pub fn save_ai_config(
    ...
    let conn = state.db.lock().map_err(|err| err.to_string())?;
    let stored_api_key = ai::get_config(&conn)
        .map_err(|err| err.to_string())?
        .api_key;
    let mut config = config.into_config(stored_api_key);
    if is_managed_local_deepseek_config(&config) {
        config.analysis_batch_size = config
            .analysis_batch_size
            .min(state.system_capabilities.recommended_analysis_batch_size);
        apply_managed_runtime_endpoint(&state, &mut config);
    }
    ai::save_config(&conn, config)
        .map(AiConfigView::from)
        .map_err(|err| err.to_string())
}
...
pub(crate) async fn runtime_config_for_analysis(
    ...
    runtime_config_for_analysis_with_recovery(app, state, config, false).await
}

pub(crate) async fn recover_runtime_config_for_analysis(
    app: &AppHandle,
    state: &AppState,
    config: &AiConfig,
) -> Result<AiConfig, String> {
    runtime_config_for_analysis_with_recovery(app, state, config, true).await
}

async fn runtime_config_for_analysis_with_recovery(
    app: &AppHandle,
    state: &AppState,
    config: &AiConfig,
    force_restart: bool,
    ...
    let base_url = ensure_local_deepseek_runtime(app, state, force_restart)
        .await
        .map_err(|err| {
...
pub async fn test_ai_connection(
    ...
    if is_managed_local_deepseek_config(&config) {
        ...
        if config.enabled {
            config.base_url = ensure_local_deepseek_runtime(&app, &state, false)
                .await
                .map_err(|err| {
...
#[tauri::command]
pub async fn get_local_deepseek_status(
    state: State<'_, AppState>,
) -> Result<LocalModelStatus, String> {
    Ok(verified_local_deepseek_status(&state).await)
}
...
fn apply_managed_runtime_endpoint(state: &AppState, config: &mut AiConfig) {
    if is_managed_local_deepseek_config(config) {
        config.base_url = state.local_model_runtime.base_url();
    }
}
...
pub fn clear_local_deepseek_model(state: State<'_, AppState>) -> Result<LocalModelStatus, String> {
    ...
    let verification_marker_path = local_deepseek_verification_marker_path(&state.app_dir);
    remove_file_if_exists(&model_path, "本地DeepSeek模型文件")?;
    remove_file_if_exists(&partial_path, "本地DeepSeek模型临时文件")?;
    remove_file_if_exists(&verification_marker_path, "本地DeepSeek模型校验标记")?;
