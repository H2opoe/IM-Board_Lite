...
fn default_bind_command_windows(platform: &str, cli_path: &Path, config_dir: &Path) -> String {
    ...
    match platform {
        ...
        "feishu" => {
            ...
            format!(
                "$env:LARKSUITE_CLI_CONFIG_DIR = {}; {} config init --new --brand feishu --name {}; if ($LASTEXITCODE -eq 0) {{ {} --profile {} auth login --scope {} }}; if ($LASTEXITCODE -eq 0) {{ {} --profile {} auth status --verify }}",
