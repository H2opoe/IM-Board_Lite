...
#### macOS: Initialize a Cloned / Multi-Instance WeChat

If you copied WeChat and changed its Bundle ID, for example:

- App: `/Applications/微信双开.app`
- Bundle ID: `com.tencent.xinWeChat2`
- Container: `~/Library/Containers/com.tencent.xinWeChat2`

Initialize that instance explicitly:

```bash
sudo wechat-cli init \
  --config /Users/chase/.wechat-cli/config_work.json \
  --keys-file /Users/chase/.wechat-cli/all_keys_work.json \
  --bundle-id com.tencent.xinWeChat2 \
  --app-path /Applications/微信双开.app \
  --force
```

Use the same config file for later queries:

```bash
wechat-cli --config /Users/chase/.wechat-cli/config_work.json sessions
```

If the container ID differs from the Bundle ID, also pass `--container-id`. If multiple `WeChat` processes are running and the app path cannot be matched automatically, pass `--pid`.
