...
如果你复制了微信应用并修改了 Bundle ID，例如：

- 应用：`/Applications/微信双开.app`
- Bundle ID：`com.tencent.xinWeChat2`
- 容器：`~/Library/Containers/com.tencent.xinWeChat2`
...
如果容器 ID 和 Bundle ID 不一致，再额外指定 `--container-id`。如果同时运行了多个名为 `WeChat` 的进程且无法自动匹配，可以指定 `--pid`。
