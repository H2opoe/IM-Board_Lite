...
DEFAULT_MACOS_BUNDLE_ID = "com.tencent.xinWeChat"
...
def _auto_detect_db_dir_macos(container_id=None):
    container_id = (
        container_id
        or os.environ.get("WECHAT_CLI_CONTAINER_ID")
        or os.environ.get("WECHAT_CLI_BUNDLE_ID")
        or DEFAULT_MACOS_BUNDLE_ID
    )
    base = os.path.expanduser(
        f"~/Library/Containers/{container_id}/Data/Documents/xwechat_files"
    )
...
def auto_detect_db_dir(bundle_id=None, container_id=None):
    if _SYSTEM == "windows":
        return _auto_detect_db_dir_windows()
    if _SYSTEM == "linux":
        return _auto_detect_db_dir_linux()
    if _SYSTEM == "darwin":
        return _auto_detect_db_dir_macos(container_id or bundle_id)
    return None
...
def load_config(config_path=None):
    ...
    bundle_id = cfg.get("wechat_bundle_id") or os.environ.get("WECHAT_CLI_BUNDLE_ID")
    container_id = (
        cfg.get("wechat_container_id")
        or os.environ.get("WECHAT_CLI_CONTAINER_ID")
        or bundle_id
    )
    ...
    if not db_dir:
        detected = auto_detect_db_dir(bundle_id=bundle_id, container_id=container_id)
        if detected:
            cfg["db_dir"] = detected
        else:
    ...
    if _SYSTEM == "darwin":
        cfg.setdefault("wechat_bundle_id", bundle_id or DEFAULT_MACOS_BUNDLE_ID)
        cfg.setdefault("wechat_container_id", container_id or cfg["wechat_bundle_id"])
